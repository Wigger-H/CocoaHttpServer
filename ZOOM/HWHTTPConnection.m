...
//  ZOOM
//
//  Created by huangwei on 2020/7/17.
//  Copyright © 2020 Weshape3D. All rights reserved.
//

#import "HWHTTPConnection.h"
...
- (NSArray *)sslIdentityAndCertificates
{
    ...
    NSString *thePath = [[NSBundle mainBundle] pathForResource:@"testHttps" ofType:@"p12"];
...
- (BOOL)supportsMethod:(NSString *)method atPath:(NSString *)path
{
    ...
    if ([method isEqualToString:@"POST"])
    {
        if ([path isEqualToString:@"/calculate"])
        {
            ...
            return YES;
        }else if ([path isEqualToString:@"/uploadFile"]){
            return YES;;
        }
    }
...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
    ...
    //加减乘除计算
    if ([method isEqualToString:@"POST"] && [path isEqualToString:@"/uploadFile"])
    {
        HTTPLogVerbose(@"%@[%p]: postContentLength: %qu", THIS_FILE, self, requestContentLength);
        NSData *requestData = [request body];
        UIImage *image = [UIImage imageWithData:requestData];
        
        NSData *responseData = [NSJSONSerialization dataWithJSONObject:@{@"message":@"保存成功"} options:0 error:nil];
        return [[HTTPDataResponse alloc] initWithData:responseData];
    }
